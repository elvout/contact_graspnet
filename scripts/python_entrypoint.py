...
def find_valid_modules() -> dict[str, ModuleType]:
    ...
    for package_info in project_packages_info:
        ...
        for submodule_info in pkgutil.iter_modules(
            ...
            if not submodule_info.name.endswith("_node"):
                continue
...
if __name__ == "__main__":
    ...
    # Python modules may specify resource locations using relative paths.
    new_working_directory = Path(__file__).parent
    print(f"[INFO] Changing node's working directory to '{new_working_directory}'")
