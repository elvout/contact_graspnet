...
def visualize_grasps(full_pc, pred_grasps_cam, scores, plot_opencv_cam=False, pc_colors=None, gripper_openings=None, gripper_width=0.08, interactive=True):
    ...
    Keyword Arguments:
        ...
        interactive {bool} -- Whether to use an interactive GUI session. If false, this function returns an image (default: {False})
    """
    ...
    if interactive:
        ...
        # generate a higher-res image since we can't manipulate the camera
        fig = mlab.figure('Pred Grasps', size=(1000, 800))
    ...
    if interactive:
        ...
        # The "distance" parameter to mlab.view didn't seem to be doing anything,
        # so we'll modify the camera zoom instead
        fig.scene.camera.zoom(1.67)
        img = mlab.screenshot(fig, antialiased=True)
        return img
